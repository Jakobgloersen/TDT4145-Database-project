...
def brukerhistorie3():
    ...
    cursor.execute(
        """insert into Billettkjop values (3, '2024-03-10', '13:00:00', 0)"""
    )
    # finner fid
    cursor.execute(
    ...
    for billett in billetter:
        cursor.execute(
            """insert into Billett values (?, ?, ?, ?, 'Gamle scene', 'Trøndelag Teater', ?, 3, 'Ordinær')""",
            (
                billettid,
                billett[0],
                rad[0],
        ...
        print(
            f"Billett-id: {billettid}. Stol: {billett[0]} Rad: {rad[0]}. Område: {rad[1]}"
        )
        billettid += 1
    con.commit()
    con.close()
